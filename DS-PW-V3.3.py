...
root.style.configure('.', font=('微软雅黑', 11))
...
def start_autohotkey_script(script_path):
    try:
        # 使用 AutoHotkey 的完整路径启动脚本
        subprocess.Popen([r"C:\Program Files\AutoHotkey\AutoHotkey.exe", script_path])
    ...
    #     subprocess.Popen(['autohotkey', '"C:/Users/Public/DS-PW.ahk"'])
    # except FileNotFoundError:
    #     # 如果找不到 AutoHotkey，则更新状态标签文本为错误消息
    #     status_label.config(text="AutoHotkey is not installed or not in the system path.")

# 创建标签显示 "New Password:"
new_password_label = tk.Label(root, text="Input DS Password:")
new_password_label.pack()

# 创建文本框用于输入新密码
new_password_entry = ttk.Entry(root, width=60)
